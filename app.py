...
# ── ADMIN: RUN MISSED TRADE FOR ONE CLIENT ────────────────────────────────────
# Add this route alongside your other /api/admin/* routes.
# Uses the SAME eligibility/payout logic as run_daily_trades(), just scoped
# to a single user_id. Safe to call even if that client already traded today —
# the UNIQUE(user_id, date) constraint + ON CONFLICT DO NOTHING makes it a
# no-op in that case rather than a double payout.

@app.route("/api/admin/trade/run-single", methods=["POST"])
@admin_required
def admin_run_single_client_trade():
    """
    Backfill today's trade for ONE client who was skipped (e.g. due to a
    DB outage during the scheduled run window).
    Body: { "user_id": "<id>" }
    """
    d   = request.json or {}
    ...
    # Already has today's row? Bail out cleanly instead of relying on the
    # DB constraint to silently no-op — this way the admin gets a clear
